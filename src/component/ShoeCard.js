function ShoeCard({imgURL , changeBigShoeImage , bigShoeImg}) {
    const handleClick =()=>{
        if(bigShoeImg !== imgURL.bigShoe){changeBigShoeImage(imgURL.bigShoe)}
    }
    var nam 
    if (bigShoeImg === imgURL.bigShoe){
        nam = "border-coral-red border-2 rounded-xl  cursor-pointer max-sm:flex-1"
    }else {
        nam = "border-transparent border-2 rounded-xl  cursor-pointer max-sm:flex-1"
    }
  return (
    <div className={nam}  onClick={handleClick}>
        <div className="flex justify-center items-center bg-card bg-center bg-cover sm:w-40 sm:h-40 rounded-xl max-sm:p-4">
            <img src={imgURL.thumbnail} alt="shoe" width={127} height={103} className="object-contain "/>
        </div>
    </div>
  )
}

export default ShoeCard